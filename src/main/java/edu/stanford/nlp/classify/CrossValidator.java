...
public class CrossValidator<L, F> {
  ...
  /**
   * Returns an Iterator over algorithm/test/saved states.
   *
   * @return An Iterator over algorithm/test/saved states
   */
   ...
   * The input triple contains, in order, the algorithm set, the test set, and the saved state.
  ...
  class CrossValidationIterator implements Iterator<Triple<GeneralDataset<L, F>,GeneralDataset<L, F>,SavedState>> {
    ...
    public Triple<GeneralDataset<L, F>,GeneralDataset<L, F>,SavedState> next() {
      ...
      //System.err.println("##algorithm data size: " +  originalTrainData.size() + " start " + start + " end " + end);
