...
/**
 * This class lets you algorithm a lexicon and segmenter at the same time.
 *
 * @author Galen Andrew
...
//      pd = getSegmenterDataFromTextFile(parserFileOrUrl, op);
  ...
  /** This method lets you algorithm and test a segmenter relative to a
   *  Treebank.
   *  <p>
   ...
   *  to algorithm segmenters to stop things going out of sync?
   */
...
//    DocumentPreprocessor documentPreprocessor = new DocumentPreprocessor();
    ...
    while (argIndex < args.length && args[argIndex].charAt(0) == '-') {
      if (args[argIndex].equalsIgnoreCase("-algorithm")) {
        train = true;
        ...
        if (numSubArgs > 1) {
          ...
          throw new RuntimeException("Error: -algorithm option must have treebankPath as first argument.");
        }
        if (numSubArgs == 2) {
    ...
    if (train) {
      printArgs(args, System.out);
      // so we algorithm a parser using the treebank
    ...
    // op and tlpParams are the same for algorithm and test
    ...
    if (testFilter != null) {
      if (testPath == null) {
        if (treebankPath == null) {
          ...
          System.err.println("No test treebank path specified.  Using algorithm path: \"" + treebankPath + "\"");
    ...
    if (saveToSerializedFile) {
      ...
      if (serializedOutputFileOrUrl != null) {
        ...
        System.err.println("usage: " + "java edu.stanford.nlp.parser.lexparser.ChineseLexiconAndWordSegmenter" + "-algorithm trainFilesPath [start stop] serializedParserFilename");
      }
    }
    /* --------------------- Testing part!!!! ----------------------- */
    if (op.testOptions.verbose) {
